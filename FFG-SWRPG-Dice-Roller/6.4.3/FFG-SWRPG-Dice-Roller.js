/*
 ...
 Semantic Versioning
 Consolidated the Dice Pool and Destiny Sections
 Collapsing sections
...
eote.process.downgrade = function (cmd, diceObj) {
    ...
    _.each(cmd, function (dice) {
        ...
        if (diceArray && diceArray[1] && diceArray[2]) {
            ...
            switch (type) {
                case 'proficiency':
                    abilityDice += Math.min(proficiencyDice, downgradeVal);
                    proficiencyDice = Math.max(0, proficiencyDice - downgradeVal);
                    diceObj.count.ability = abilityDice;
                    diceObj.count.proficiency = proficiencyDice;
                    break;
                case 'challenge':
                    difficultyDice += Math.min(challengeDice, downgradeVal);
                    challengeDice = Math.max(0, challengeDice - downgradeVal);
                    diceObj.count.difficulty = difficultyDice;
                    diceObj.count.challenge = challengeDice;
                    break;
            }
        }
    });
